...
# Configure logging - reduce verbosity in combined script mode
quiet_mode = os.getenv('CHAINLIT_QUIET') == '1'
log_level = logging.WARNING if quiet_mode else logging.INFO
logging.basicConfig(level=log_level, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')

# Suppress HTTP client logs in quiet mode
if quiet_mode:
...
async def start_chat():
    ...
    # Ensure user_env is a dictionary to prevent AttributeError
    if not isinstance(user_env, dict):
        logger.warning(f"user_env is not a dictionary, resetting to empty dict. Type: {type(user_env)}, Value: {user_env}")
        user_env = {} 
